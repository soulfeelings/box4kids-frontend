    ...
    <div className="p-4" style={{ 
      ...
      <div className="p-4 mb-4" style={{ backgroundColor: '#F0955E', borderRadius: '24px' }}>
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-white font-medium">Текущий набор • {getCurrentDate()}</h2>
        </div>
        
        <div className="space-y-2 mb-4">
          {allToys.map((toy, index) => (
            <div key={index} className="flex items-center text-white">
              <div className="w-6 h-6 rounded-full mr-3 flex items-center justify-center text-xs" style={{ backgroundColor: '#F8CAAF' }}>
                {toy.icon}
              </div>
