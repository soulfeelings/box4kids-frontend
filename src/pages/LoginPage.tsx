...
export const LoginPage: React.FC<LoginPageProps> = ({
  ...
  const handleCategoriesSubmit = async () => {
    ...
    if (editingChildId && editMode === "data") {
      ...
      try {
        ...
        if (childIdToUpdate) {
          const numericChildId = parseInt(childIdToUpdate);
  ...
  const handleSubscriptionSubmit = async () => {
    ...
    try {
      ...
      if (editingChildId && editMode === "subscription") {
        ...
        if (child) {
          const childIdNum = parseInt(child.id);
          if (!isNaN(childIdNum)) {
            await createSubscription(childIdNum, selectedPlan.id);
          }
        }
  ...
  const handleEditChildSubscription = async (childId: string) => {
    const child = children.find((c) => c.id === childId);
    if (child) {
